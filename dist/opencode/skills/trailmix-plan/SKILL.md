...
`trailmix-trailhead/refs/trail-metadata.md`) before planning. On the **trivial track** you're
...
trivial work merged by `trailmix-trailhead`, scaffold `trail.mjs new <slug> spec-plan "<title>"`
and use `refs/spec-plan-template.md` instead (one artifact, one checkpoint). See
`trailmix-trailhead/refs/trail-metadata.md`.

## Checkpoint
Get the human's sign-off on the plan before `trailmix-implement`. Sign-off passed: a good point
